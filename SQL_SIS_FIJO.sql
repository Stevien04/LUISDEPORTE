...
CREATE TABLE tbEstadoInvitacion (
    IdEstadoInvitacion INT IDENTITY(1,1) PRIMARY KEY,
    NombreEstado VARCHAR(50) NOT NULL UNIQUE,
    Descripcion VARCHAR(255)
);
GO
INSERT INTO tbEstadoInvitacion (NombreEstado, Descripcion)
VALUES
('Pendiente', 'Invitación enviada y aún no respondida'),
('Aceptada', 'El usuario aceptó la invitación y debe agregarse como miembro'),
('Rechazada', 'El usuario rechazó la invitación'),
('Expirada', 'La invitación venció sin respuesta');
GO
...
CREATE TABLE tbInvitacionEquipo (
    ...
    FOREIGN KEY (IdEstadoInvitacion) REFERENCES tbEstadoInvitacion(IdEstadoInvitacion)
);
...
CREATE PROCEDURE sp_BuscarUsuariosParaInvitar
    @IdEquipo INT,
...
BEGIN
    ...
    SELECT TOP 20
        ...
        P.Correo,
        I.IdInvitacion,
        EI.NombreEstado AS EstadoInvitacion,
        I.FechaEnvio,
        I.FechaRespuesta
    FROM tbUsuario U
    INNER JOIN tbPersona P ON U.IDPersona = P.IDPersona
    LEFT JOIN tbInvitacionEquipo I
        ON I.IdEquipo = @IdEquipo AND I.IdUsuarioCreador = U.IDUsuario
    LEFT JOIN tbEstadoInvitacion EI ON I.IdEstadoInvitacion = EI.IdEstadoInvitacion
    WHERE U.Estado = 1
        AND (
            P.Nombres LIKE '%' + @TextoBusqueda + '%'
            OR P.ApellidoPaterno LIKE '%' + @TextoBusqueda + '%'
            OR P.ApellidoMaterno LIKE '%' + @TextoBusqueda + '%'
            OR U.NombreUsuario LIKE '%' + @TextoBusqueda + '%'
        )
        AND U.IDUsuario NOT IN (
            SELECT idUsuarioMiembro FROM tbEquipoMiembros WHERE idEquipo = @IdEquipo
        )
